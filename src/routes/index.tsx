...
const projects = [
  { id: "01", title: "ZOPPER PRINTABLES STUDIO", category: "BUILD", eyebrow: "PRODUCT / FULL-STACK / INTERNAL TOOL", copy: "An internal tool built for Zopper’s marketing team to turn design PDFs into press-ready printable files.", tags: ["PRODUCT", "FULL-STACK", "INTERNAL TOOL", "AI"], images: [zopper1.url, zopper2.url], cta: "VIEW DETAILS", link: "https://zopper-printables-studio.vercel.app/" },
  { id: "02", title: "DIGGIN CAFÉ", category: "DESIGN", eyebrow: "WEBSITE / DIGITAL EXPERIENCE", copy: "A complete digital experience for a café — from the interface to the ordering experience and backend setup.", tags: ["WEBSITE", "UI/UX", "FULL-STACK"], images: [diggin1.url, diggin2.url], cta: "VISIT SITE", link: "https://diggin-india.vercel.app/" },
  { id: "03", title: "OUTBOUND SALES AUTOMATION", category: "AUTOMATE", eyebrow: "AUTOMATION / AI VOICE / SALES", copy: "An AI-powered outbound sales workflow combining voice conversations, automation and lead processing.", tags: ["AI", "VOICE", "AUTOMATION"], images: [], cta: "WATCH WALKTHROUGH", link: null },
  { id: "04", title: "SHIV AI", category: "CREATE", eyebrow: "AI AGENT / REAL ESTATE", copy: "A real-estate AI assistant that talks back.", tags: ["AI AGENT", "TELEGRAM", "VOICE", "n8n"], images: [shiv1.url, shiv2.url], cta: "VIEW DETAILS", link: "https://app.notion.com/p/Shiv-24-7-Real-Estate-Agent-2ad61a9951c480eabb77c36d21d5ebd3?source=copy_link" },
] as const;

const experiments = [
  { title: "INVENTORY AUTOMATION", copy: "Structured inventory workflows and AI-assisted operations.", image: inventory.url, alt: "Inventory automation workflow", link: null },
  { title: "DIGIDZN", copy: "Digital agency experience and admin interface.", image: digidzn.url, alt: "DigiDZN website interface", link: "https://digi-dzn-shiv-automates.vercel.app/" },
] as const;
...
function ProjectCard({ project, onImage }: { project: (typeof projects)[number]; onImage: (src: string, alt: string) => void }) {
  const cta = project.link
    ? <Button asChild variant="pill"><a href={project.link} target="_blank" rel="noreferrer" data-cursor="external">{project.cta} <ArrowUpRight /></a></Button>
    : <Button variant="pill">{project.cta} <ArrowUpRight /></Button>;
  return <article className={`project project-${project.id}`} data-cursor="view"><div className="project-copy"><span className="project-no">{project.id}</span><p className="eyebrow">{project.eyebrow}</p><h3>{project.title}</h3><p>{project.copy}</p><div className="tags">{project.tags.map(tag => <span key={tag}>{tag}</span>)}</div>{cta}</div><div className="project-media">{project.images.length ? project.id === "04" ? <div className="telegram-showcase">{project.images.map((src, i) => { const alt = `Shiv AI Telegram conversation ${i + 1}`; return <Button key={src} variant="icon" className="phone-frame" onClick={() => onImage(src, alt)} aria-label={`Enlarge ${alt}`}><span className="phone-speaker"/><img src={src} alt={alt} loading="lazy"/><span className="phone-home"/></Button>; })}</div> : project.images.map((src, i) => { const alt = `${project.title} interface ${i + 1}`; return <Button key={src} variant="icon" className="project-shot" onClick={() => onImage(src, alt)} aria-label={`Enlarge ${alt}`}><img src={src} alt={alt} loading="lazy" /></Button>; }) : <OutboundVisual />}</div></article>;
}
...
function Index() {
  ...
  return <main id="top">
    ...
    <Reveal><section id="playground" className="playground"><SectionHead kicker="12 / PLAYGROUND" title="PLAYGROUND" copy="A space for ideas, experiments and random things I’m exploring."/><div className="experiment-grid">{experiments.map(item => { const inner = <><img src={item.image} alt={item.alt} loading="lazy"/><h3>{item.title}</h3><p>{item.copy}</p>{item.link && <span className="experiment-cta">VISIT SITE <ArrowUpRight /></span>}</>; return item.link ? <a key={item.title} className="experiment" href={item.link} target="_blank" rel="noreferrer" data-cursor="external">{inner}</a> : <article key={item.title} className="experiment">{inner}</article>; })}</div></section></Reveal>
